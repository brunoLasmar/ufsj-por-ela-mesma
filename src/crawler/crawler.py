...
def extrair_texto_noticia(urls: list[str]): # para cada notícia, extrai seu título, data de publicação e seu conteúdo.
    ...
    for url in urls:
        try:
            ...
            data_publicacao = paragrafos[0].text
            texto_completo = []
            texto_completo = "\n".join(paragrafo.text for paragrafo in paragrafos[1:])
    ...
    with open("data/raw/noticias.json", "w", encoding="utf8") as file:
...
def extrair_texto_boletins(urls: list[str]):
        ...
        try:
            ...
            for linha in linhas:
                ...
                if re.search(r"^\s*\d+\s*[-–—]", linha):
                    if noticia_atual:
                        noticia_atual["texto"] = noticia_atual["texto"].strip()
    ...
    with open("data/raw/boletins.json", "w", encoding="utf-8") as file:
        json.dump(dataset, file, ensure_ascii=False, indent=4)
